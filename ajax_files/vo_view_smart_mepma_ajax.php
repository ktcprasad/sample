...
if(!isset($_SESSION['TRANS_VO_ID']) && !isset($_SESSION['desk_deo_id']))
{
	exit("Validation: Session Expired, Please login again");
        ...
        if($_SERVER[REMOTE_ADDR]=='182.19.66.187' || $_SERVER[REMOTE_ADDR]== '202.56.197.65')
         fwrite($fp,"\n------TEST----Project 72---".date("Y-m-d H:i:s a"));
        else
         fwrite($fp,"\n----------Project 72---".date("Y-m-d H:i:s a"));
         fclose($fp);
...
$check_shg_samrudhi=$total_deposit_rs['SHG_ID'];
	...
	$credit_limit_pop_ig=ceil($vo_limit_pop*0.85);
	  ...
	  $shg_mem_rs=mssql_query("select * from IVRS_LOAN_REQUEST(nolock) where  SHG_ID='$shg_code' and PROJECT_TYPE='72' and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'");
	  $mem_pending=mssql_num_rows($shg_mem_rs);	
	  
/*	$shg_mem_rs_live=mssql_query("select * from IVRS_LOAN_REQUEST_LIVE(nolock) where  SHG_ID='$shg_code' and PROJECT_TYPE='72' and IVRS_ID='$ivr_call_id'");
	 $mem_pending_live=mssql_num_rows($shg_mem_rs_live);
*/	  
     $shg_mem_rej_rs=mssql_query("select VO_ID from SHG_LOAN_APPLICATION(nolock) where STATUS_ID='11' and  SHG_ID='$shg_code' and PROJECT_TYPE='72' ");
	 $mem_rejected=mssql_num_rows($shg_mem_rej_rs);	
	 
	 $msquery="select smlsn.VO_ID FROM SAMSNBSTG.SN.SHG_MEMBER_LOAN_STATUS_NEW smlsn,shg_loan_application(nolock) sla WHERE smlsn.SHG_ID='$shg_code' and smlsn.SHG_MEMBER_LOAN_ACCNO=sla.SHG_MEM_LOAN_ACCNO and sla.PROJECT_TYPE='72' and smlsn.IS_CLOSED='1'";
...
//		$member_limit=$max_loans-$mem_pending_live+$mem_rejected+$repaid_loans_members;   
	   ...
	   //////// adding serp validation start 25-06-18 for accessing variables shg_max_loans_ivrs,shg_max_loans_total
...
//////// adding serp validation end 25-06-18 for accessing variables shg_max_loans_ivrs,shg_max_loans_total
   
           $member_limit=shg_member_limit($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$health,$vo_shgs,72,$shg_code,$shg_max_loans_total,$shg_max_loans_ivrs);
...
list_members($void_code,$shg_code);
		$amt_stat='Y';
//	request_loan_mepma($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$db_filed,$type,$length,$play_msg,$shg_code,$shg_name,$amt_stat,$is_over_due,$due_id_lst,$dist_id,$mandal_id,$health,$vo_shgs,$curr_odos);
	       }
		   else
		   {
	...
	echo   $message="<b>Validation:</b> FAIL: Morethan ".$shg_max_loans_total." loans |Members already having Loans : $member_limit[1]";
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
		   }
}
...
$shg_oustanding_memmbers .= $shg_oustanding_project_array[MEMBER_SHORT_CODE].",";
                ...
                $message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
                $shg_limit_project=$shg_max_loans_total-$shg_oustanding_project_rows;
...
//      echo $message="Validation: "."UPDATE SHG_MEMBER_INFO SET MOBILE_NUM='$memberMobNo'  where SHG_ID='$shg_code' and member_id='$memberId'";exit;
        if($_SERVER[REMOTE_ADDR]!='182.19.66.187' && $_SERVER[REMOTE_ADDR]!= '202.56.197.65'&& $_SERVER[REMOTE_ADDR]!= '202.56.197.65'&& $_SERVER[REMOTE_ADDR]!= '125.16.9.129')
...
$member_id_rs=mssql_fetch_array(mssql_query("select MEMBER_ID,MOBILE_NUM,AGE,SHORT_CODE,IS_POP_MEM from SHG_MEMBER_INFO(nolock)  where SHG_ID='$shg_code' and MEMBER_ID='$memberId' and IS_MEPMA='Y'"));
 ...
 $member_short_code=trim($member_id_rs['SHORT_CODE']);
   $member_type=$member_id_rs['IS_POP_MEM'];
	...
	$member_prev_loan_count = mssql_num_rows(mssql_query("select VO_ID from IVRS_LOAN_REQUEST(nolock) where SHG_ID='$shg_code' and MEMBER_ID='$member_id' and PROJECT_TYPE ='72' and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'"));
	
/*	$member_prev_loan_count_live = mssql_num_rows(mssql_query("select VO_ID from IVRS_LOAN_REQUEST_LIVE(nolock) where SHG_ID='$shg_code' and MEMBER_ID='$member_id' and PROJECT_TYPE ='72' and IVRS_ID='$ivr_call_id'"));
*/	
	$member_rej_cnt_lng = mssql_num_rows(mssql_query("select VO_ID from SHG_LOAN_APPLICATION(nolock) where SHG_ID='$shg_code' and MEMBER_LONG_CODE='$member_id'  and STATUS_ID='11' and  PROJECT_TYPE='72' "));
...
//	php_log_ivr($ivr_call_id,$message);
	...
	//$member_loans_tcs=mssql_num_rows(mssql_query("select VO_ID from IVRS_LOAN_REQUEST(nolock) where SHG_ID='$shg_code' and ( MEMBER_ID='$member_id'  or SHORT_CODE='$member_short_code') and  PROJECT_TYPE='72' and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'"));
	
	$member_loans_tcs=mssql_num_rows(mssql_query("select VO_ID from IVRS_LOAN_REQUEST(nolock) where SHG_ID='$shg_code' and MEMBER_ID='$member_id' and  PROJECT_TYPE='72'"));

/*	$member_loans_evol=mssql_num_rows(mssql_query("select VO_ID from IVRS_LOAN_REQUEST_LIVE(nolock) where SHG_ID='$shg_code' and MEMBER_ID='$member_id' and  PROJECT_TYPE='72'"));
*/
...
select I.VO_ID,I.SHG_ID,I.IVRS_ID,I.PROJECT_TYPE,I.MEMBER_ID,I.SHORT_CODE,SLA.STATUS_ID,SMLS.IS_CLOSED,sla.MEMBER_SHORT_CODE from evgen.IVRS_LOAN_REQUEST I (nolock) left join SHG_LOAN_APPLICATION SLA (nolock) on I.IVRS_ID=SLA.REQUESTED_ID and SLA.SHG_ID=I.SHG_ID and isnull(I.MEMBER_ID,'')=isnull(SLA.MEMBER_LONG_CODE,'') left join SAMSNBSTG.SN.SHG_MEMBER_LOAN_STATUS_NEW(nolock) SMLS on SMLS.SHG_MEMBER_LOAN_ACCNO=sla.SHG_MEM_LOAN_ACCNO and sla.project_type in ('5','72') and SMLS.IS_CLOSED='1' where I.shg_id like '$shg_code' and I.member_id='$member_id' and I.PROJECT_TYPE in('5') and I.MEMBER_ID is not null  and isnull(I.IS_PROCESSED,'NOTDUPLICATE') !='D'  ) A where (A.STATUS_ID<>'11'or a.STATUS_ID is null) and (A.IS_CLOSED is null or A.IS_CLOSED<>'1') ";
*/
$distinct_member_ids_exist_query = "SP_SMARTPHONE_LOANSVALIDATION '$shg_code','$member_id','72','--','$shg_max_loans_total'";
...
$distinct_member_ids_exist_row = mssql_fetch_array($distinct_member_ids_exist_result);
        $message="SP_SMARTPHONE_LOANSVALIDATION '$shg_code','$member_id','72','--','$shg_max_loans_total'--$distinct_member_ids_exist_row[STATUS]";
         $message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
...
exit("Validation: Member has overdue : $member_overdue_row[LOAN_DUE]");
				...
				{
				exit("Validation: FAIL: Morethan ".$shg_max_loans_total." loans");
...
if($diff_loan_amt=='0')
		{
		 if($loan_amount<=6000 && $loan_amount>=1000)
		     {
		//return $loan_amount;
		           }
				   else
				   {
					if($loan_amount>6000)
					{
					echo "Validation: Loan amount should not be more than 6000";exit;
...
$vo_grade_rs_rej=mssql_fetch_array(mssql_query("select A.ACTUAL_CREDIT_LIMIT+ISNULL(B.ACTUAL_CREDIT_LIMIT,0) AS ACTUAL_CREDIT_LIMIT,A.POP_CREDIT_LIMIT,A.NONPOP_CREDIT_LIMIT,GEN_LOAN_PER from VO_CREDIT_LIMIT(nolock) A LEFT JOIN VO_CREDIT_LIMIT25K(NOLOCK) B ON A.VO_ID=B.VO_ID where A.VO_ID='$void_code' and A.IS_MEPMA='Y'"));
	$vo_actual_credit=$vo_grade_rs_rej['ACTUAL_CREDIT_LIMIT'];
	$vo_limit_pop=$vo_grade_rs_rej['POP_CREDIT_LIMIT'];
 	$vo_limit_npop=$vo_grade_rs_rej['NONPOP_CREDIT_LIMIT'];
 	$vo_gen_loan_per=$vo_grade_rs_rej['GEN_LOAN_PER'];
...
/*		$mms_lt_live_rs=mssql_fetch_array(mssql_query("select SUM(LOAN_AMOUNT) from IVRS_LOAN_REQUEST_LIVE(nolock) where DISTRICT_ID='$mms_dist_code' and MANDAL_ID='$mms_mandal_code' and IVRS_ID='$ivr_call_id' and PROJECT_TYPE='72'"));*/
...
//$vo_outstanding=vo_outstanding($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$health,$vo_shgs,'72');
...
//$vo_outstanding_tcs=vo_outstanding_tcs($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$health,$vo_shgs,'72');
...
if($vo_category_drawing_power >= $loan_amount && $loan_amount<=$vo_drawing_power  && $loan_amount <= $tcs_shg_drawing_power)
  {
 $shg_ivr_loan_rs=mssql_fetch_array(mssql_query("select * from IVRS_LOAN_REQUEST(nolock) where vo_id='$void_code' and  SHG_ID='$shg_code' and PROJECT_TYPE='72' order by CREATED_DATE  desc "));
...
echo "Validation: member short code:$member_short_code|member prev loan count:$member_prev_loan_count|member id:$member_id|member type:$member_type";
exit;
}
	...
	if($current_limit_nonpop_non_ig>0)
{
	...
	function shg_outstanding_amt($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$health,$vo_shgs,$project,$shg_code){
		...
		$shg_applied_project_rs=mssql_fetch_array(mssql_query("select sum(loan_amount) from IVRS_LOAN_REQUEST(nolock) where shg_id='$shg_code' and PROJECT_TYPE ='72' and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'"));
		$shg_applied_project=$shg_applied_project_rs[0];
		
		$TCS_shg_applied_project_rs=mssql_fetch_array(mssql_query("select sum(actual_amount) from shg_loan_application(nolock) where shg_id='$shg_code' and PROJECT_TYPE ='72'"));
		$TCS_shg_applied_project=$TCS_shg_applied_project_rs[0];
		
		$shg_rejected_project_rs=mssql_fetch_array(mssql_query("select sum(actual_amount) from shg_loan_application(nolock) where shg_id='$shg_code' and PROJECT_TYPE ='72' and STATUS_ID='11'"));
		$shg_rejected_project=$shg_rejected_project_rs[0];
		
		$shg_repaid_project_rs=mssql_fetch_array(mssql_query("select sum(sla.ACTUAL_AMOUNT) FROM SAMSNBSTG.SN.SHG_MEMBER_LOAN_STATUS_NEW(nolock) smlsn,shg_loan_application(nolock) sla WHERE smlsn.SHG_ID='$shg_code' and smlsn.SHG_MEMBER_LOAN_ACCNO=sla.SHG_MEM_LOAN_ACCNO and sla.project_type ='72' and smlsn.IS_CLOSED='1'"));
		$shg_repaid_project=$shg_repaid_project_rs[0];
		...
		$shg_applied_project_tcs_rs=mssql_fetch_array(mssql_query("select sum(ACTUAL_AMOUNT) from shg_loan_application(nolock) where shg_id='$shg_code' and PROJECT_TYPE ='72' and REQUESTED_ID='201314'"));
