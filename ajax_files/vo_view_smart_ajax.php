...
if(!isset($_SESSION['TRANS_VO_ID']) && !isset($_SESSION['desk_deo_id']))
{
	...
	log_ivr($ivr_call_id,"------TEST---Project 72---".date("Y-m-d H:i:s a"));
	else	
	log_ivr($ivr_call_id,"----LIVE---Project 72---".date("Y-m-d H:i:s a"));
...
$search_purpose_non_ig="'Education','Marriage','Emergency Needs/Health','Emergency Needs'";
	...
	list_members($void_code,$shg_code);
		   ...
		   {
		  echo   $message="Validation: FAIL: Morethan ".$shg_max_loans_total." loans | Members already having Loans : $member_limit[1]";
	   $message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);	
...
SELECT DISTINCT MEMBER_ID MEMBER_LONG_CODE,SHORT_CODE as MEMBER_SHORT_CODE  FROM EVGEN.IVRS_LOAN_REQUEST (NOLOCK) WHERE SHG_ID ='$shg_code'
...
//		$shg_limit_project=$shg_max_loans_ivrs-$shg_oustanding_project;// max smartphone loans is 9
		$shg_limit_project=$shg_max_loans_total-$shg_oustanding_project;
...
select I.VO_ID,I.SHG_ID,I.IVRS_ID,I.PROJECT_TYPE,I.MEMBER_ID,I.SHORT_CODE,SLA.STATUS_ID,SMLS.IS_CLOSED,sla.MEMBER_SHORT_CODE
from evgen.IVRS_LOAN_REQUEST I (nolock) 
left join SHG_LOAN_APPLICATION SLA (nolock) on I.IVRS_ID=SLA.REQUESTED_ID and SLA.SHG_ID=I.SHG_ID and isnull(I.MEMBER_ID,'')=isnull(SLA.MEMBER_LONG_CODE,'') left join SAMSNBSTG.SN.SHG_MEMBER_LOAN_STATUS_NEW(nolock) SMLS on SMLS.SHG_MEMBER_LOAN_ACCNO=sla.SHG_MEM_LOAN_ACCNO and sla.project_type in ('1','72') and SMLS.IS_CLOSED='1' where I.shg_id like '$shg_code' and I.member_id='$member_id' and I.PROJECT_TYPE in('1') and I.MEMBER_ID is not null  and isnull(I.IS_PROCESSED,'NOTDUPLICATE') !='D'  ) A where (A.STATUS_ID<>'11'or a.STATUS_ID is null) and (A.IS_CLOSED is null or A.IS_CLOSED<>'1') ";
*/
...
$distinct_member_ids_exist_row = mssql_fetch_array($distinct_member_ids_exist_result);
        $message="SP_SMARTPHONE_LOANSVALIDATION '$shg_code','$member_id','72','--','$shg_max_loans_ivrs'--$distinct_member_ids_exist_row[STATUS]";
        $message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
...
############	Member Validation new End project type 1
 if($member_age > 63 || $member_age < 18){
	echo "Validation: Member Age should be between 18 to 63 ";
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
	...
	$member_prev_loan_count_tcs = mssql_num_rows(mssql_query("select VO_ID from SHG_LOAN_APPLICATION(nolock) where  SHG_ID='$shg_code' and MEMBER_LONG_CODE='$member_id' and STATUS_ID!='99'  and project_type='72'"));
...
if($diff_loan_amt=='0')
 ...
 if($member_type=='Y')
	...
	$applied_rs=mssql_fetch_array(mssql_query("select sum(LOAN_AMOUNT) as AMT from IVRS_LOAN_REQUEST(nolock)  where shg_id in ($vo_shgs) and IS_POP='$member_type' and PROJECT_TYPE ='72'  and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'")); 

	
        $applied_amt = $applied_rs['AMT'];
		
/*		$applied_rs_live=mssql_fetch_array(mssql_query("select sum(LOAN_AMOUNT) as AMT_LIVE from IVRS_LOAN_REQUEST_LIVE(nolock)  where shg_id in ($vo_shgs) and IS_POP='$member_type' and IVRS_ID='$ivr_call_id' and PROJECT_TYPE ='72' "));
        $applied_amt_live = $applied_rs_live['AMT_LIVE'];
		$applied_amt_live=intval($applied_amt_live);
*/
		
		$vo_rej_amt_rs=mssql_fetch_array(mssql_query("select SUM(LOAN_AMOUNT) as AMT_REJ from SHG_LOAN_APPLICATION(nolock) where shg_id in ($vo_shgs) and IS_POP='$member_type' and PROJECT_TYPE ='72'   and STATUS_ID='11'"));		
...
//print "Validation:$vo_category_drawing_power >= $loan_amount && $loan_amount<=$vo_drawing_power  && $vo_drawing_power_tcs >= $loan_amount && $loan_amount <= $tcs_shg_drawing_power";
 ...
 $shg_ivr_loan_rs=mssql_fetch_array(mssql_query("select * from IVRS_LOAN_REQUEST(nolock) where  SHG_ID='$shg_code' and project_type='72' order by CREATED_DATE  desc "));
...
function shg_outstanding_amt($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$health,$vo_shgs,$project,$shg_code){
		...
		/*$shg_applied_project_live_rs=mssql_fetch_array(mssql_query("select sum(loan_amount) from IVRS_LOAN_REQUEST_LIVE(nolock) where shg_id='$shg_code' and PROJECT_TYPE  IN (SELECT PROJECT_TYPE FROM GET_PTYPES(0)) and IVRS_ID='$ivr_call_id'"));
		$shg_applied_project_live=$shg_applied_project_live_rs[0];
		*/
		...
		$shg_applied_project_tcs_rs=mssql_fetch_array(mssql_query("select sum(loan_amount) from SHG_LOAN_APPLICATION(nolock) where shg_id='$shg_code' and PROJECT_TYPE ='72'  and REQUESTED_ID='201314'"));
...
*/		
	$vo_repaid_total_rs=mssql_fetch_array(mssql_query("select sum(PPR) as AMT_REPAID_VO from SN.SHG_MEMBER_REPAY_VIEW(nolock)  where shg_id in ($vo_shgs)  and PROJECT_TYPE IN (SELECT PROJECT_TYPE FROM GET_PTYPES(0)) and project_type not in('71','72','74','43')"));
		...
		$vo_credit_max_tcs=$vo_lt_max_tcs+$vo_lt_max_tcs_pending+$vo_lt_live-$vo_repaid_total-$vo_amt_to;
		 ...
		 $message = "vo_credit_max_tcs=vo_lt_max_tcs+vo_lt_max_tcs_pending+vo_lt_live-vo_repaid_total-vo_amt_to::$vo_credit_max_tcs=$vo_lt_max_tcs+$vo_lt_max_tcs_pending+$vo_lt_live-$vo_repaid_total-$vo_amt_to\n";
                 $message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
		return $vo_credit_max_tcs;
		}	
