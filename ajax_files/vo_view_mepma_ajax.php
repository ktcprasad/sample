...
if(!isset($_SESSION['TRANS_VO_ID']) && !isset($_SESSION['desk_deo_id']))
{
	...
	if($_SERVER[REMOTE_ADDR]=='182.19.66.187' || $_SERVER[REMOTE_ADDR]== '202.56.197.65')
         fwrite($fp,"\n------TEST----Project 5---".date("Y-m-d H:i:s a"));
	else	
         fwrite($fp,"\n-----LIVE-----Project 5---".date("Y-m-d H:i:s a"));
         fclose($fp);
...
$total_deposit_rs=mssql_fetch_array(mssql_query("VO_MEETING_DAY_VALIDATION '$vo_id'"));
        ...
        { echo $message="Validation:  Please update meeting date.";
...
//Disabling web screen to A,B and C grade VO's after 15 days of dispatch of tablet PC end 26-11-2018
		...
		//$agi->Hangup();
		//exit("Validation: Loan Not allowed for this district, Please select other district");//allowing loans as per request by CM-MIS 15-02-18
		//mssql_close($link1);
		}
		//STOPING NIZAMABAD,all SLFs for all loans in  Armoor TLF
...
$project = 5 ;
	...
	//Restricting loan request if no. of members lessthan 5 in a SHG -start 29-11-18
			...
			if($shg_members <5){
			echo $message="Validation:No. of members in a SHG is lessthan 5";
			log_ivr($ivr_call_id,$message);
			exit;
			}	
	//Restricting loan request if no. of members lessthan 5 in a SHG - End 29-11-18
...
//	echo $message="Validation: Bank Linkage AMT $overdue_amt is gretaer than 10000"; exit;
	echo $message="Validation: Overdue AMT($overdue_amt) in Bank Linkage is greater than 10000 "; 
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
	exit;
	}	
...
$check_shg_samrudhi= check_shg_samrudhi($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$health,$vo_shgs,$shg_code);
	...
	if($mms_grade=='E')
	{
	echo	$message="Validation: MMS grade $mms_grade";
		$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
	exit;
...
/*	
	  $shg_mem_rs=mssql_query("select * from IVRS_LOAN_REQUEST(nolock) where  SHG_ID='$shg_code' and PROJECT_TYPE='5' and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'");
	  $mem_pending=mssql_num_rows($shg_mem_rs);	
	  
     $shg_mem_rej_rs=mssql_query("select VO_ID from SHG_LOAN_APPLICATION(nolock) where STATUS_ID='11' and  SHG_ID='$shg_code' and PROJECT_TYPE='5' ");
	 $mem_rejected=mssql_num_rows($shg_mem_rej_rs);	
	 
	 $msquery="select smlsn.VO_ID FROM SAMSNBSTG.SN.SHG_MEMBER_LOAN_STATUS_NEW smlsn,shg_loan_application(nolock) sla WHERE smlsn.SHG_ID='$shg_code' and smlsn.SHG_MEMBER_LOAN_ACCNO=sla.SHG_MEM_LOAN_ACCNO and sla.project_type='5' and smlsn.IS_CLOSED='1'";
...
*/
	 ...
	 $mem_count=mssql_num_rows($shg_members_rs);	
	  $max_loans = 6;
		if($mem_count>=10)
		{
			// $max_loans =8; //Restrict maximum members to 6 in MEPMA(Monday, February 19, 2018)
		}
                $shg_max_loans_total = $max_loans;
...
//////// adding serp validation start 02-05-18 for accessing variables shg_max_loans_ivrs,shg_max_loans_total
...
//////// adding serp validation end 02-05-18 for accessing variables shg_max_loans_ivrs,shg_max_loans_total

//exit("Validation: shg_member_limit($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$health,$vo_shgs,$project,$shg_code,$shg_max_loans_total,$shg_max_loans_ivrs)");
	 ...
	 $message="Member Limit:$member_limit";
        $message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);	    
...
//	$this->request_loan_mepma($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$db_filed,$type,$length,$play_msg,$shg_code,$shg_name,$amt_stat,$is_over_due,$due_id_lst,$dist_id,$mandal_id,$health,$vo_shgs,$curr_odos);
		   ...
		   {
	//echo   $message="Validation: FAIL: member_limit: $member_limit Less Than 1 , Morethan $max_loans loans ";
		echo   $message="<b>Validation:</b> FAIL: Morethan ".$shg_max_loans_ivrs." loans |Members already having Loans : $member_limit[1]";
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
...
function shg_limits($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$project,$shg_code){
			...
			elseif($shg_members == 7 || $shg_members == 8 || $shg_members == 9 ){
				$shg_max_loans_total=6;
				$shg_max_loans_ivrs=4;
				$shg_max_credit_limit=200000;
			}
			elseif($shg_members == 5 || $shg_members == 6){
				$shg_max_loans_total=4;
				$shg_max_loans_ivrs=2;
				$shg_max_credit_limit=150000;
			}
...
$shg_oustanding_memmbers .= $shg_oustanding_project_array[MEMBER_SHORT_CODE].",";
         ...
         $fp = fopen("Logs/$shg_code.txt","a");
         fwrite($fp,"\n shg_allowed_loans : $shg_allowed_loans(shg_max_loans_ivrs-shg_oustanding_project_rows : $shg_max_loans_total-$shg_oustanding_project_rows)");
         fclose($fp);
...
//////// adding serp validation end 02-05-18 for accessing variables shg_max_loans_ivrs,shg_max_loans_total
	   $member_limit=shg_member_limit($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$health,$vo_shgs,$project,$shg_code,$shg_max_loans_total,$shg_max_loans_ivrs);
		...
		if($member_limit[0]<1)
		  {
		  //echo $message="Validation: FAIL: Morethan ".$shg_max_loans_total." loans "; 
		  		  echo   $message="<b>Validation:</b> FAIL: Morethan ".$shg_max_loans_ivrs." loans |Members already having Loans : $member_limit[1]";
		  	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
		  exit;
		  }
...
$member_id_rs=mssql_fetch_array(mssql_query("select MEMBER_ID,MOBILE_NUM,AGE,SHORT_CODE,IS_POP_MEM from SHG_MEMBER_INFO(nolock)  where SHG_ID='$shg_code' and MEMBER_ID='$memberId' and IS_MEPMA='Y'"));
 ...
 if($member_age > 63 && $member_age != 0){
	echo "Validation: Member Age is Greater than 63 ";exit;
	...
	$member_prev_loan_count = mssql_num_rows(mssql_query("select VO_ID from IVRS_LOAN_REQUEST(nolock) where SHG_ID='$shg_code' and MEMBER_ID='$member_id' and PROJECT_TYPE ='5' and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'"));
	
/*	$member_prev_loan_count_live = mssql_num_rows(mssql_query("select VO_ID from IVRS_LOAN_REQUEST_LIVE(nolock) where SHG_ID='$shg_code' and MEMBER_ID='$member_id' and PROJECT_TYPE ='5' and IVRS_ID='$ivr_call_id'"));
*/	
	$member_rej_cnt_lng = mssql_num_rows(mssql_query("select VO_ID from SHG_LOAN_APPLICATION(nolock) where SHG_ID='$shg_code' and MEMBER_LONG_CODE='$member_id'  and STATUS_ID='11' and  PROJECT_TYPE='5' "));
...
$message="Member details:  MEMBER_ID: $member_id , SHG ID: $shg_code, Short Code:  $memberId,member_uid: $member_uid , member_age:$member_age , IS_POP_MEM:$IS_POP_MEM , member_mobile_num: $member_mobile_num, Loan_type: $loan_type, Reason_loan: $reason_loan, Loan_amount: $loan_amount \n(".($member_prev_loan_count-$member_rej_cnt_sht-$member_repaid_loans).")member_prev_loan_count:$member_prev_loan_count-member_rej_cnt_sht:$member_rej_cnt_sht-member_repaid_loans:$member_repaid_loans";
...
//	log_ivr($ivr_call_id,$message);	
	...
	//$member_loans_tcs=mssql_num_rows(mssql_query("select VO_ID from IVRS_LOAN_REQUEST(nolock) where SHG_ID='$shg_code' and ( MEMBER_ID='$member_id'  or SHORT_CODE='$member_short_code') and  PROJECT_TYPE='5' and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'"));
	
	$member_loans_tcs=mssql_num_rows(mssql_query("select VO_ID from IVRS_LOAN_REQUEST(nolock) where SHG_ID='$shg_code' and MEMBER_ID='$member_id' and  PROJECT_TYPE='5'"));

/*	$member_loans_evol=mssql_num_rows(mssql_query("select VO_ID from IVRS_LOAN_REQUEST_LIVE(nolock) where SHG_ID='$shg_code' and MEMBER_ID='$member_id' and  PROJECT_TYPE='5'"));
*/
...
//	$this->php_log_ivr($ivr_call_id,$message);
	...
	$member_count_query="select * from streenidhi_member_loan_count_tab('$shg_code','$member_id')";
	...
	$message="member previous loan count from procedure".$member_prev_loan_count;
	log_ivr($ivr_call_id,$message);	
	...
	$member_prev_loan_count=0;
       ...
       /* if($member_prev_loan_count != 0 )
	{
	//echo "Validation: ".$message="Member loan_already_applied : member_prev_loan_count:".$member_prev_loan_count;
	echo "Validation: ".$message="Loan already applied by this member";
	exit;
...
*/
	...
	if($member_prev_loan_count == 1)
        ...
        if($overdue != 0 && $instno < 21||$overdue == 0 && $instno <21||$overdue != 0 && $instno>=21)
         ...
         $message="OVERDUE:----".$overdue." and INSTALLMENTS:".$instno;
        log_ivr($ivr_call_id,$message);
 	echo "Validation: ".$message="Member has  not completed 21 installments in micro";
	exit;
         
         }
	}
         else{
          echo "Validation: ".$message="Member Has Already Applied Loan";
		exit;
          log_ivr($ivr_call_id,$message);
         }
          }
...
left join SAMSNBSTG.SN.SHG_MEMBER_LOAN_STATUS_NEW(nolock) SMLS on SMLS.SHG_MEMBER_LOAN_ACCNO=sla.SHG_MEM_LOAN_ACCNO and sla.project_type in ('5','72') and SMLS.IS_CLOSED='1'
where I.shg_id like '$shg_code' and I.PROJECT_TYPE in('5','72') and I.MEMBER_ID is not null
...
*/
$distinct_member_ids_exist_query = "SP_SMARTPHONE_LOANSVALIDATION '$shg_code','$member_id','5','--','$shg_max_loans_ivrs'";
...
$distinct_member_ids_exist_row = mssql_fetch_array($distinct_member_ids_exist_result);
        ...
        $message="SP_SMARTPHONE_LOANSVALIDATION '$shg_code','$member_id','5','--','$shg_max_loans_ivrs'--$distinct_member_ids_exist_row[STATUS]";
        $message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
                                if($distinct_member_ids_exist_row[STATUS]=='LOAN ALLOWED')
                                {
$member_overdue_query="select LOAN_DUE from SHG_LOAN_APPLICATION(nolock) a,SAMSNBSTG.SN.SHG_MEMBER_LOAN_STATUS_NEW(nolock) b where a.SHG_ID='$shg_code' and a.MEMBER_LONG_CODE='$member_id' and a.SHG_MEM_LOAN_ACCNO=b.SHG_MEMBER_LOAN_ACCNO and a.PROJECT_TYPE=72  and b.IS_CLOSED='0'";
...
$aa = "<pre>".print_r($member_overdue_row,1).count($member_overdue_row)."</pre>";
		...
		{
exit("Validation: Member has overdue : $member_overdue_row[LOAN_DUE]");
				if($x>='1')
					{
						...
						#$agi-> stream_file($wfile, '#');
						$x=$x-1;
						member_loan($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$shg_max_loans_ivrs,$db_filed,$type,$length,$play_msg,$shg_code,$shg_name,$amt_stat,$is_over_due,$due_id_lst,$health,$vo_shgs,$shg_max_credit_limit,$curr_odos);
						mssql_close($link);
						$agi->Hangup();
		  				exit;
		   			}else
		   			{
						...
						mssql_close($link);
		   				...
		   				exit;
                                ...
                                else if($distinct_member_ids_exist_row[STATUS]=='MEMBER HAVE ALREADY LOAN')
        {
exit("Validation: Member already have the loan in Sthreenidhi or Other Sthrreenidhi");
        }
				else
				{
				exit("Validation: FAIL: Morethan ".$shg_max_loans_ivrs." loans");
...
############	Member Validation new End project type 1
		...
		if($loan_type == ''){
			echo "Validation: Please select Loan Type";
			exit;
		}
		...
		if($reason_loan == ''){
			echo "Validation: Please select Reason Loan";
			exit;
		}
...
//Ashok change for purpose general
			...
			{ 
				echo "Validation: notgeneral-purpose";
...
/*       //$db_filed="category";
 ...
 //if(($reason_loan=='Agriculture'||$reason_loan=='Dairy'||$reason_loan=='Income generation Activity'||$reason_loan=='Emergency Needs/Health'||$reason_loan=='Education'||$reason_loan=='Marriage'||$reason_loan=='Weavers')&&($member_type=='Y'||$member_type=='N')&&(strlen($member_short_code)==2 && strlen($member_id)>1 ))
...
{
echo "Validation: Loan Amount should not be greater than 20000";exit;
...
{
echo "Validation: Loan Amount should not be greater than 15000";exit;
...
//////// adding serp validation start 02-05-18
		$shg_limit_array=shg_limits($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$project,$shg_code);
		$shg_max_loans_total=$shg_limit_array[$shg_code]['shg_max_loans_total'];
		$shg_max_loans_ivrs=$shg_limit_array[$shg_code]['shg_max_loans_ivrs'];
		$shg_max_credit_limit=$shg_limit_array[$shg_code]['shg_max_credit_limit'];
//////// adding serp validation end 02-05-18

//$tcs_shg_drawing_power=150000-$tcs_shg_outstanding_amt;
$tcs_shg_drawing_power=$shg_max_credit_limit-$tcs_shg_outstanding_amt;


 $message="VALIDATING SHG Drawing power tcs_shg_drawing_power: $shg_max_credit_limit = $shg_max_credit_limit - tcs_shg_outstanding_amt: $tcs_shg_outstanding_amt";
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
    ...
    $mms_total_credit=$mms_total_credit_rs[0];
		...
		$mms_lt_max_tcs_rs=mssql_fetch_array(mssql_query("select SUM(LOAN_AMOUNT) from IVRS_LOAN_REQUEST(nolock) where DISTRICT_ID='$mms_dist_code' and MANDAL_ID='$mms_mandal_code' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74') and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'"));
		$mms_lt_max_tcs=$mms_lt_max_tcs_rs[0];
		

/*		$mms_lt_live_rs=mssql_fetch_array(mssql_query("select SUM(LOAN_AMOUNT) from IVRS_LOAN_REQUEST_LIVE(nolock) where DISTRICT_ID='$mms_dist_code' and MANDAL_ID='$mms_mandal_code' and IVRS_ID='$ivr_call_id' and PROJECT_TYPE='5'"));*/
		...
		$mms_rejected_rs=mssql_fetch_array(mssql_query("select SUM(ACTUAL_AMOUNT) from SHG_LOAN_APPLICATION(nolock) where  DISTRICT_ID='$mms_dist_code' and MANDAL_ID='$mms_mandal_code' and STATUS_ID='11' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74')"));
...
//$vo_outstanding=vo_outstanding($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$health,$vo_shgs,'5');
...
//$vo_outstanding_tcs=vo_outstanding_tcs($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$health,$vo_shgs,'5');
...
//  if($vo_category_drawing_power >= $loan_amount && $loan_amount<=$vo_drawing_power  && $vo_drawing_power_tcs >= $loan_amount && $loan_amount <= $tcs_shg_drawing_power)
 ...
 $shg_ivr_loan_rs=mssql_fetch_array(mssql_query("select * from IVRS_LOAN_REQUEST(nolock) where vo_id='$void_code' and  SHG_ID='$shg_code' and PROJECT_TYPE='5' order by CREATED_DATE  desc "));
...
if($_SERVER[REMOTE_ADDR]=='182.19.66.187' || $_SERVER[REMOTE_ADDR]== '202.56.197.65' || $_SERVER[REMOTE_ADDR]=='125.16.9.129' ||  $_SERVER[REMOTE_ADDR]=='125.16.9.132')
exit("Validation: $_SERVER[REMOTE_ADDR]----$insert_query--");
...
/*
 ...
 $vo_amt_to_add_rs=mssql_fetch_array(mssql_query("select SUM(ACTUAL_AMOUNT) from SHG_LOAN_APPLICATION(nolock)  where shg_id in ($vo_shgs) and  PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74') and STATUS_ID='11'"));
       $vo_amt_to=$vo_amt_to_add_rs[0];

 	$message="Rejected Amount SHG_LOAN_APPLICATION (select SUM(ACTUAL_AMOUNT) from SHG_LOAN_APPLICATION(nolock)  where shg_id in ($vo_shgs) and  PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74') and STATUS_ID='11') , vo_amt_to: $vo_amt_to  ";
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);

		
		$vo_lt_max_tcs_rs=mssql_fetch_array(mssql_query("select SUM(LOAN_AMOUNT) from IVRS_LOAN_REQUEST(nolock) where shg_id in ($vo_shgs) and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74') and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'"));
		$vo_lt_max_tcs=$vo_lt_max_tcs_rs[0];
		
		 	$message="Applied Loan Amount from IVRS_LOAN_REQUEST (select SUM(LOAN_AMOUNT) from IVRS_LOAN_REQUEST(nolock) where shg_id in ($vo_shgs) and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74')) and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D' \n vo_lt_max_tcs :$vo_lt_max_tcs ";
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
...
* /
	...
	$vo_repaid_total_rs=mssql_fetch_array(mssql_query("select sum(PPR) as AMT_REPAID_VO from SN.SHG_MEMBER_REPAY_VIEW(nolock)  where shg_id in ($vo_shgs)  and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74')"));
        $vo_repaid_total = $vo_repaid_total_rs['AMT_REPAID_VO'];
		$vo_repaid_total=intval($vo_repaid_total);
		
	$message="Repaid Loan Amount from SN.SHG_MEMBER_REPAY_VIEW (select sum(PPR) as AMT_REPAID_VO from SN.SHG_MEMBER_REPAY_VIEW(nolock)  where shg_id in ($vo_shgs)  and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74'))  vo_repaid_total :$vo_repaid_total ";
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
...
$applied_pop_rs=mssql_fetch_array(mssql_query("select sum(LOAN_AMOUNT) as AMT from IVRS_LOAN_REQUEST(nolock)  where shg_id in ($vo_shgs) and IS_POP='Y' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74') and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'"));
...
$vo_rej_amt_rs=mssql_fetch_array(mssql_query("select SUM(ACTUAL_AMOUNT) as AMT_REJ from SHG_LOAN_APPLICATION(nolock) where shg_id in ($vo_shgs) and IS_POP='Y' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74') and STATUS_ID='11'"));
$vo_rej_amt_pop=$vo_rej_amt_rs['AMT_REJ'];
$vo_rej_amt_pop=intval($vo_rej_amt_pop);


	$message="POP Rejected AMT (select SUM(ACTUAL_AMOUNT) as AMT_REJ from SHG_LOAN_APPLICATION(nolock) where shg_id in ($vo_shgs) and IS_POP='Y' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74') and STATUS_ID='11')   vo_rej_amt_pop:$vo_rej_amt_pop ";
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);

//non pop------
		

$applied_npop_rs=mssql_fetch_array(mssql_query("select sum(LOAN_AMOUNT) as AMT from IVRS_LOAN_REQUEST(nolock)  where shg_id in ($vo_shgs) and IS_POP='N' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74') and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'"));
$applied_amt_nonpop=$applied_npop_rs['AMT'];


	$message="NON POP Applied AMT (select sum(LOAN_AMOUNT) as AMT from IVRS_LOAN_REQUEST(nolock)  where shg_id in ($vo_shgs) and IS_POP='N' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74')) and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D' applied_amt_nonpop:$applied_amt_nonpop ";
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
...
$vo_rej_amt_nonpop_rs=mssql_fetch_array(mssql_query("select SUM(ACTUAL_AMOUNT) as AMT_REJ from SHG_LOAN_APPLICATION(nolock) where shg_id in ($vo_shgs) and IS_POP='N' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74') and STATUS_ID='11'"));
$vo_rej_amt_nonpop=$vo_rej_amt_nonpop_rs['AMT_REJ'];
$vo_rej_amt_nonpop=intval($vo_rej_amt_nonpop);

	$message="NON POP Rejected AMT (select SUM(ACTUAL_AMOUNT) as AMT_REJ from SHG_LOAN_APPLICATION(nolock) where shg_id in ($vo_shgs) and IS_POP='N' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74') and STATUS_ID='11')   vo_rej_amt_nonpop:$vo_rej_amt_nonpop ";
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
			
$vo_repaid_pop_rs=mssql_fetch_array(mssql_query("select sum(PPR) as AMT_REPAID_POP from SN.SHG_MEMBER_REPAY_VIEW(nolock)  where shg_id in ($vo_shgs) and IS_POP='0' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74')"));
        $vo_repaid_pop = $vo_repaid_pop_rs['AMT_REPAID_POP'];
		
	$message="POP Repaid AMT (select sum(PPR) as AMT_REPAID_POP from SN.SHG_MEMBER_REPAY_VIEW(nolock)  where shg_id in ($vo_shgs) and IS_POP='0' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74'))   vo_repaid_pop:$vo_repaid_pop ";
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);		
		
		
		$vo_repaid_nonpop_rs=mssql_fetch_array(mssql_query("select sum(PPR) as AMT_REPAID_NONPOP from SN.SHG_MEMBER_REPAY_VIEW(nolock)  where shg_id in ($vo_shgs) and IS_POP='1' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74')"));
        $vo_repaid_nonpop = $vo_repaid_nonpop_rs['AMT_REPAID_NONPOP'];
	$message="NONPOP Repaid AMT (select sum(PPR) as AMT_REPAID_NONPOP from SN.SHG_MEMBER_REPAY_VIEW  where shg_id in ($vo_shgs) and IS_POP='1' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74'))   vo_repaid_nonpop:$vo_repaid_nonpop ";
	$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);	
	...
	if($current_limit_nonpop_non_ig>0)
{
	...
	function shg_outstanding_amt($caller,$exten,$starttime,$agi,$time,$x,$unique_id,$ivr_call_id,$language,$void_code,$member_limit,$health,$vo_shgs,$project,$shg_code){
		...
		$shg_applied_project_rs=mssql_fetch_array(mssql_query("select sum(loan_amount) from IVRS_LOAN_REQUEST(nolock) where shg_id='$shg_code' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74','43') and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D'"));
		$shg_applied_project=$shg_applied_project_rs[0];
		
		$message="shg_applied_project: $shg_applied_project :  select sum(loan_amount) from IVRS_LOAN_REQUEST(nolock) where shg_id='$shg_code' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74','43') and isnull(IS_PROCESSED,'NOTDUPLICATE') !='D' ";
		$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
...
/*		$shg_applied_project_live_rs=mssql_fetch_array(mssql_query("select sum(loan_amount) from IVRS_LOAN_REQUEST_LIVE(nolock) where shg_id='$shg_code' and PROJECT_TYPE='$project' and IVRS_ID='$ivr_call_id'"));
		$shg_applied_project_live=$shg_applied_project_live_rs[0];
*/		
		$shg_rejected_project_rs=mssql_fetch_array(mssql_query("select sum(actual_amount) from shg_loan_application(nolock) where shg_id='$shg_code' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74','43') and STATUS_ID='11'"));
		$shg_rejected_project=$shg_rejected_project_rs[0];
		
		$shg_repaid_project_rs=mssql_fetch_array(mssql_query("select sum(sla.ACTUAL_AMOUNT) FROM SAMSNBSTG.SN.SHG_MEMBER_LOAN_STATUS_NEW(nolock) smlsn,shg_loan_application(nolock) sla WHERE smlsn.SHG_ID='$shg_code' and smlsn.SHG_MEMBER_LOAN_ACCNO=sla.SHG_MEM_LOAN_ACCNO and sla.project_type IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74','43') and smlsn.IS_CLOSED='1'"));
		$shg_repaid_project=$shg_repaid_project_rs[0];
		
		$message="Repaid AMOUNT of SHG in project $project  select sum(sla.ACTUAL_AMOUNT) FROM SAMSNBSTG.SN.SHG_MEMBER_LOAN_STATUS_NEW(nolock) smlsn,shg_loan_application sla WHERE smlsn.SHG_ID='$shg_code' and smlsn.SHG_MEMBER_LOAN_ACCNO=sla.SHG_MEM_LOAN_ACCNO and sla.project_type IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74','43') and smlsn.IS_CLOSED='1'";
		$message=str_replace(')','^',$message);$message=str_replace('(','^',$message);log_ivr($ivr_call_id,$message);
		
		
		$shg_applied_project_tcs_rs=mssql_fetch_array(mssql_query("select sum(ACTUAL_AMOUNT) from shg_loan_application(nolock) where shg_id='$shg_code' and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74','43') and REQUESTED_ID='201314'"));
...
*/		
	$vo_repaid_total_rs=mssql_fetch_array(mssql_query("select sum(PPR) as AMT_REPAID_VO from SN.SHG_MEMBER_REPAY_VIEW(nolock)  where shg_id in ($vo_shgs)  and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74')"));
		...
		$applied_loans_tcs_rs=mssql_fetch_array(mssql_query("select SUM(ACTUAL_AMOUNT) from SHG_LOAN_APPLICATION(nolock)  where shg_id in ($vo_shgs) and  PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74') and REQUESTED_ID='201314'"));
	  ...
	  // $vo_total_credit=$vo_total_credit+$vo_amt_to;
		...
		$vo_lt_max_tcs_rs=mssql_fetch_array(mssql_query("select SUM(ACTUAL_AMOUNT) from SHG_LOAN_APPLICATION(nolock) where shg_id in ($vo_shgs) and PROJECT_TYPE IN (select PROJECT_TYPE FROM GET_PTYPES(0)) and PROJECT_TYPE not in('71','72','74')"));
		$vo_lt_max_tcs=$vo_lt_max_tcs_rs[0];
